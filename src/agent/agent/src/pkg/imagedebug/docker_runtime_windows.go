...
func configureImageDebugDockerRunner(runner *dockercli.Runner, winOptions *api.WinOptions) error {
	options, enabled, err := imageDebugProcessOptions(winOptions)
	if err != nil {
		return err
	}
	if !enabled {
...
func imageDebugProcessOptions(winOptions *api.WinOptions) (winprocess.Options, bool, error) {
	if winOptions == nil {
