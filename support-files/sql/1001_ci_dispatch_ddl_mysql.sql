...
CREATE TABLE IF NOT EXISTS `T_DISPATCH_THIRDPARTY_AGENT_BUILD` (
  ...
  `TIME_INTERVAL` bigint(20) NULL COMMENT '时间间隔',
  `START_USER` varchar(32) NULL COMMENT '流水线启动者',
  `STAGE_ID` varchar(64) NULL COMMENT '步骤ID',
  `OPTIONS` json NULL COMMENT '构建参数',
  PRIMARY KEY (`ID`),
  UNIQUE KEY `BUILD_ID` (`BUILD_ID`,`VM_SEQ_ID`),
...
CREATE TABLE IF NOT EXISTS `T_DISPATCH_THIRDPARTY_AGENT_QUEUE` 
(
  `ID` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `PROJECT_ID` varchar(64) NOT NULL COMMENT '项目ID',
  `PIPELINE_ID` varchar(34) NOT NULL COMMENT '流水线ID',
  `BUILD_ID` varchar(34) NOT NULL COMMENT '构建ID',
  `VM_SEQ_ID` varchar(34) NOT NULL COMMENT '构建序列号',
  `DATA` varchar(256) NOT NULL COMMENT '排队资源，随着类型不同而不同',
  `DATA_TYPE` varchar(64) NOT NULL COMMENT '排队资源类型',
  `INFO` json NOT NULL COMMENT '额外的事件信息',
  `RETRY_TIME` int(11) NOT NULL COMMENT '重试次数',
  `CREATED_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `UPDATE_TIME` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`ID`),
  KEY `IDX_PROJECT_DATA` (`PROJECT_ID`,`PIPELINE_ID`,`DATA`,`DATA_TYPE`,`CREATED_TIME`) USING BTREE,
